...
def resolve_m3u8_link(url, headers=None):
    ...
    # Verifica se ̬ un URL di vavoo.to
    ...
    try:
        with requests.Session() as session:
            ...
            # Se ̬ un URL di vavoo.to, salta la logica dell'iframe
            if is_vavoo:
                if initial_response_text and initial_response_text.strip().startswith('#EXTM3U'):
                    ...
                    # Se non ̬ un M3U8 diretto, restituisci l'URL originale per vavoo
                    print(f"URL vavoo.to non ̬ un M3U8 diretto: {url}")
                    return {
                        "resolved_url": url,
                        "headers": current_headers
                    }
